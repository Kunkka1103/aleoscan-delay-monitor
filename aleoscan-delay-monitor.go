...
var product = flag.String("product", "", "Product name")

func main() {
	flag.Parse()

	if *pushAddr == "" || *pgDSN == "" || *product == "" {
		log.Fatalln("push-address、pgdsn、product must be provided")
	}
	...
	for {
		...
		prometh.Push(*pushAddr, *product+"_cur_height", height)
		prometh.Push(*pushAddr, *product+"_cur_timestamp_delay_second", delay)
